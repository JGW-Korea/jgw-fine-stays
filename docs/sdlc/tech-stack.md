...
MySQL과 Sequelize ORM을 도입한 이유는 클라이언트에서 전달한 데이터를 영구적으로 저장하면서, 질의어(Query)가 아닌 프로그래밍 언어로 데이터베이스를 관리하여 생산성을 높이기 위함입니다.
...
AWS S3는 프론트엔드 결과물을 포함한 정적 자원을 저장하고, CloudFront를 통해 CDN을 구축하여 정적 자원을 더 빠르게 조회할 수 있도록 도입했습니다.

기존에는 클라이언트에서 서버로 전송한 이미지와 같은 정적 자원만을 저장하기 위해 AWS S3와 CloudFront를 도입하려 했고, 프론트엔드 개발 환경은 Vercel을 통해 배포하려 했습니다. 이는 제가 아직 AWS S3를 활용해본 경험이 없었기 때문에 내린 판단이었습니다.

물론 프론트엔드 결과물을 AWS S3를 통해 배포할 수 있다는 점은 이전 PICKY 프로젝트에서 진행을 해봤으니 알고는 있었습니다. 다만 AWS S3 자체적으로 정적 자원의 종류를 분리를 시킬 수 있는 방법을 제공되는지 알지 못했기 때문에 Vercel과 AWS S3를 같이 활용할 생각이었습니다.

그러나 AWS S3에 정적 자원을 저장하더라도 Vercel을 통해 배포한 것과 동작 자체는 바뀌지 않는다고 생각해 의문을 가졌습니다. 이후 AWS S3에 대한 정보를 확인하던 중 버킷(Bucket)을 통해 자원을 분리할 수 있다는 점을 확인했습니다.

다만 처음에는 AWS S3를 활용해본 적이 없었기 때문에 처음 생각한 대로 전달받은 정적 자원은 AWS S3에, 프론트엔드 결과물은 Vercel을 통해 배포하려 했습니다. 그러나 아직 자세히 알지 못하다는 이유로 경험하지 않는 것보다 직접 경험해보고 AWS S3의 이해도를 높이는 것이 저의 역량을 높일 수 있다고 판단하여, 프론트엔드 결과물을 포함한 정적 자원을 AWS S3만 활용하기로 결정했습니다.

<br />

### AWS EC2

AWS EC2는 백엔드 결과물을 배포하기 위해 도입했습니다.

서버를 배포하기 위한 방식에는 물리적인 컴퓨터를 서버 환경으로 구성하는 온프레미스(On-Premises), 컴퓨터 자원을 제공받아 서버 환경을 구축하는 클라우드 서비스(Cloud Service), 유휴 상태로 대기하다가 API 호출 시에만 작업을 처리하는 서버리스(Serverless) 방식이 있습니다.

온프레미스(On-Premises) 방식은 물리적인 컴퓨터를 서버 환경으로 구성할 수 있기 때문에, 현재 사용 중인 개인 컴퓨터로도 서버를 구동할 수 있어 트래픽이 발생하지 않으면 비용을 절감할 수 있습니다. 또한 서버리스(Serverless)를 활용하면 유휴 상태로 대기하다가 API 호출이 발생했을 때 작업을 처리하고, 해당 작업 수만큼 비용이 청구되기 때문에 온프레미스와 마찬가지로 트래픽이 발생하지 않으면 비용을 절감할 수 있습니다.

다만 트래픽이 몰리는 상황이 발생하면, 온프레미스(On-Premises)의 경우에는 물리적인 컴퓨터가 여러 대 필요하고, 서버리스(Serverless)의 경우 작업 처리량이 증가함에 따라 비용이 예측하기 어려워질 수 있다는 문제가 있습니다.

반면 클라우드 서비스(Cloud Service)의 경우 24시간 동안 활성화된 상태로 운영되며, 트래픽이 물리더라도 물리적인 컴퓨터 자원을 제공받아 서버 확장이 용이하고, 자원에 제한을 두어 한계를 설정할 수 있습니다. 다만 24시간 자원을 할당받기 때문에 일정 수준의 비용이 지속적으로 발생하며, 서버 확장 시 동일한 환경을 구성하기 위해 Docker와 같은 컨테이너를 사용해야 할 수도 있습니다.

이처럼 추가적인 환경 구성이 필요함에도 불구하고 클라우드 서비스(Cloud Service)인 AWS EC2를 도입한 이유는, 트래픽이 몰리더라도 사용자에게 안정적인 서비스를 제공하기 위한 서버 확장이 가능하고, 예측하기 어려운 비용 문제로 인한 기업의 부담을 줄일 수 있다고 판단했기 때문입니다.
...
데이터베이스를 배포하기 위해 AWS RDS를 도입했습니다.
